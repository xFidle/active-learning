...
\subsection{Inicjalizacja zbioru etykietowanego}
Na początku uczenia aktywnego wyznaczany jest początkowy zbiór etykietowany,
który stanowi bazę dla pierwszej iteracji modelu. Wariant \textit{random}
realizuje losowy dobór próbek, natomiast inicjalizacja \textit{cluster}
ukierunkowuje wybór na różne regiony przestrzeni cech, tak aby zapewnić lepszą
reprezentatywność zbioru startowego. Taki dobór zwiększa prawdopodobieństwo
uwzględnienia różnych klas i przyspiesza stabilizację wczesnych estymacji
modelu.

\subsection{Sposoby wybierania próbek}
W uczeniu aktywnym należy zdefiniować kryterium, według którego model będzie
wybierał próbki przeznaczone do anotacji. Przygotowane zostały trzy warianty:
\begin{itemize}
	\item \textbf{uncertainty sampling} \textendash{} największa niepewność predykcji:
	      \begin{equation*}
		      \max_{x \in P} \; \left( 1 - \max_{\hat{y} \in C}{P(\hat{y}|x)} \right)
	      \end{equation*}

	\item \textbf{diversity sampling} \textendash{} największa różnorodność:
	      \begin{equation*}
		      \max_{x \in P} \; \left( \min_{x^\prime \in T} \delta({x, x^\prime}) \right)
	      \end{equation*}
	      gdzie $\delta({x, x^\prime})$ \textendash{} miara podobieństwa między dwoma próbkami (np.\ euklidesowa).
	\item \textbf{random} \textendash{} losowo, zgodnie z rozkładem jednostajnym.
\end{itemize}

\section{Opis algorytmów}
\subsection{Random forest} Las losowy jest algorytmem,
...
\subsection{Uczenie aktywne}
\subsubsection{Założenia}
Podczas przeprowadzania eksperymentów dotczyczących uczenia aktywnego poczynione
zostały założenia:
\begin{itemize}
	\item Próbki nie są ręcznie etykietowane. Informacja o obecności etykiety
	      jest kodowana za pomocą flagi binarnej (0 \textendash{} brak etykiety, 1
	      \textendash{} etykieta obecna). Po każdym wywołaniu selektora, który
	      zwraca próbki do poetykietowania, flaga binarna dla danego przykładu
	      jest zmieniana z 0 na 1. Na podstwie ustalonych bitów model decyduje,
	      który przykład może zostać wykorzystany w zbiorze treningowym;
	\item Do ewaluacji wyników wykorzystano $n$-krotną waldiację krzyżową
	      ($k$-fold), z parametrami $n = 5$ i $k = 5$. Pondato upewniono się, że
	      w każdym zbiorze testowym znajduje się co najmniej jeden przykład z
	      klasy mniejszościowej;
	\item Jako miarę oceny jakości wykorzystano pole pod krzywą
	      \textit{precsion-recall} (PR-AUC), ponieważ skupia się ona na jakości
	      predykcji klasy pozytywnej, a nie licznych przykładach klasy
	      większościowej;
	\item Każdy selektor, który wskazuje dane do poetykietowania, zwraca je w
	      grupach liczących dokładnie 10 próbek;
	\item Wszystkie elementy losowe (np.\ generowanie drzew w lesie losowym) są
	      przeprowadzane z odpowiednio ustawionymi ziarnami dzięki czemu wyniki są
	      reprodukowalne.
\end{itemize}

\subsubsection{Zagregowane wyniki}
\begin{table}[H]
	\centering
	\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrrrrr@{}}
		...
		\bottomrule
	\end{tabular*}
	\caption{Uśrednione wartości PR-AUC na różnych etapach uczenia.}
\end{table}

Wyniki pokazują, że warianty SVM uzyskują wyższe wartości PR-AUC już przy
25\% poetykietowanych danych, a ich przewaga utrzymuje się również przy 50\%
i 75\% etykiet. Najszybszy przyrost jakości w obu modelach zapewnia selekcja
\textit{uncertainty}, co widać szczególnie dla inicjalizacji klastrowej, gdzie
SVM przekracza 0.8 PR-AUC już przy 30\%. Dla lasu losowego poprawa jest bardziej
umiarkowana i zależna od strategii: losowe próbkowanie rośnie najwolniej, a
strategie \textit{uncertainty} i \textit{diversity} wyraźnie przyspieszają naukę
w środkowej fazie (50\%--75\%).

\subsubsection{Wykresy zbieżności}
\begin{figure}[H]
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/forest-random-learning.png}
	\end{subfigure}
	\begin{subfigure}{0.49\linewidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/forest-cluster-learning.png}
	\end{subfigure}
	\caption{Krzywe uczenia w zależności od \% poetykietowanych danych dla lasu
		losowego.}
\end{figure}
...
\begin{figure}[H]
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/svm-random-learning.png}
	\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/svm-cluster-learning.png}
	\end{subfigure}
	\caption{Krzywe uczenia w zależności od \% poetykietowanych danych dla SVM.}
\end{figure}

\noindent Wnioski:
\begin{itemize}
	\item Wybieranie próbek, co do których model jest najbardziej niepewny daje
	      najlepsze wyniki, ponieważ taka selekcja w pełni wykorzystuje informacje
	      zawarte w zbiorze danych niepoetykietowanych. Zarówno dla lasu
	      losowego jak i SVM model już po paru iteracjach procesu uczenia
	      aktywnego osiąga wyniki powyżej progu wyznaczonego przez model
	      nauczony na 100\% danych;
	\item Strategie \textit{diversity} i \textit{random} nie osiągają
	      zadowalających wyników w ramach uczenia aktywnego \textendash{} prawie
	      najwyższą wartość miary jakości uzyskują dopiero przy 100\%
	      poetykietowanych danych. Oznacza to, że proces etykietowania nie może
	      zostać przerwany na wcześniejszym etapie bez istotnej utraty jakości
	      modelu;
	\item Strategia \textit{diversity} radzi sobie słabo, ponieważ w przypadku
	      tak dużej liczby atrybutów (2048) musimy mierzyć się z problemem
	      wymiarowości. W tak dużej przestrzeni zanikają różnice niepodobieństwa
	      przez co próbki wybierane są prawie że losowo;
	\item Selektor losowy (\textit{random}) w żaden sposób nie wykorzystuje
	      informacji o modelu lub zbiorze niepoetykietowanych danych przez co większość
	      nowych przykładów nie wnosi istotnej informacji i w żaden sposób nie poprawia
	      jakości predykcji.
\end{itemize}

\subsubsection{Przykładowe krzywe \textit{precision-recall}}
\begin{figure}[H]
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/forest_random_uncertainty-pr.png}
	\end{subfigure}
	\begin{subfigure}{0.49\linewidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/forest_cluster_uncertainty-pr.png}
	\end{subfigure}
	\caption{Krzywe \textit{precision-recall} dla lasu losowego.}
\end{figure}

\begin{figure}[H]
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/svm_random_uncertainty-pr.png}
	\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../results/plots/svm_cluster_uncertainty-pr.png}
	\end{subfigure}
	\caption{Krzywe \textit{precision-recall} dla SVM.}
\end{figure}
\noindent Wnioski:
\begin{itemize}
	\item Klastrowa inicjalizacja powoduje niestabilność na początkowych etapach
	      uczenia, które objawiają się jako szpilki na wykresie.
	\item Nie zawsze przy 100\% poetykietowanych danych model zwraca największe
	      wartości miary jakości (PR AUC). W przypadku strategii
	      \textit{uncertainty} model jest już ``nasycony'' przy 50\%
	      przetworzonych daynch, dalsze trenowanie nie zwiększa jakości.
	\item Wszystkie wykresy są stosunkowo gładkie \textendash{} nia ma żadnego
	      wyraźnego załamania, po którym precyzja malałaby drastycznie. Oznacza
	      to, że proces uczenia przebiega stabilnie.
\end{itemize}
...
\begin{itemize}
	...
	\item \texttt{data\_processing.unlabeled\_\allowbreak{}percentage} \textendash{} odsetek
	      przykładów z ukrytymi etykietami;
